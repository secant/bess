...
fw1 = ACL(rules=[
    {'src_ip': '172.12.0.0/16', 'drop': False},
    {'dst_ip': '192.168.32.4/32', 'dst_port': 4455, 'src_ip': '134.54.33.2/32', 'drop':False}, 
    {'src_ip':'133.133.133.0/24', 'src_port': 43, 'dst_ip':'96.96.96.155/32', 'dst_port':9, 'drop':False}])
...
fw4 = ACL(rules=[{'src_ip':'96.0.0.0/8', 'drop':False}]) #You can also run two tests back to back.
...
## Here you can just define your own functions and link pipelines together. If your test succeeds, return 0.
## If your test fails, return any other number and the test framework will report a failure.

#This test is kind of stupid -> Its the same as the crash test, only with some custom traffic I specified.
